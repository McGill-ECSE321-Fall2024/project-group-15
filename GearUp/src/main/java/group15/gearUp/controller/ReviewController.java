...
import group15.gearUp.service.ReviewService;
...
public class ReviewController{
    ...
    public ResponseEntity<ReviewDto> createReview(@RequestBody ReviewDto reviewDto) {
        Equipment equipment = equipmentService.getEquipmentByID(reviewDto.getEquipmentId());
        ...
        Review createdReview = reviewService.createReview(reviewDto.getRating(),
            reviewDto.getDescription(), equipment,customer);
     ...
     * @param EquipmentId the ID of the Equipment for which to retrieve reviews
     * @return desired review information and the HTTP status "OK"
     */
    @GetMapping("/review/Equipment/{EquipmentId}")
    public ResponseEntity<List<ReviewDto>> getReviewByEquipment(@PathVariable Equipment equipment) {
        List<Review> reviews = reviewService.getReviewByEquipment(equipment);
        List<ReviewDto> responseDtoList = reviews.stream().map(ReviewDto::new).collect(Collectors.toList());
        return new ResponseEntity<>(responseDtoList, HttpStatus.OK);  
    }
