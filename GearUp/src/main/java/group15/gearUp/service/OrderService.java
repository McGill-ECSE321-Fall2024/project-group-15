...
import group15.gearUp.repository.OrderRepository;
...
public class OrderService {
     ...
     * @throws GearUpException if the order with the given ID is not found
     */
    public Order getOrderByID(int orderID) {
        Order order = orderRepo.findOrderByOrderID(orderID);
        if (order == null) {
            throw new GearUpException(HttpStatus.NOT_FOUND, String.format("There is no order with ID %d", orderID));
        }
        return order;
    }
     ...
     * @throws GearUpException if there are no orders in the system
     */
    public List<Order> getAllOrders() {
        List<Order> orders = orderRepo.findAll();
        if (orders.isEmpty()) {
            throw new GearUpException(HttpStatus.NOT_FOUND, "There are no orders in the system");
        }
        return orders;
    }
    ...
    public Order createOrder(String orderNumber, Status orderStatus, double price, Customer customer) {
        if (orderNumber == null || orderNumber.trim().isEmpty() || orderStatus == null) {
            throw new GearUpException(HttpStatus.BAD_REQUEST, "Order number or Order Status is required.");
        }
        if (price < 0) {
            throw new GearUpException(HttpStatus.BAD_REQUEST, "The price of an order cannot be negative.");
        }
        if (customer == null || customerRepo.findByUserID(customer.getUserID()) == null) {
            throw new GearUpException(HttpStatus.NOT_FOUND, "The specified customer does not exist.");
        }
     ...
     * @throws GearUpException if the order or employee is not found or the request is invalid
     */
    @Transactional
    public void deleteOrder(Order orderToDelete, Employee employee) {
        if (orderToDelete == null || employee == null) {
            throw new GearUpException(HttpStatus.BAD_REQUEST, "Invalid order deletion request: missing attributes");
        }
        if (employeeRepo.findByUserID(employee.getUserID()) == null) {
            throw new GearUpException(HttpStatus.NOT_FOUND, String.format("The employee '%s' that made the request does not exist", employee.getUsername()));
        }
    ...
    public Order updateOrder(int orderID, Order updatedOrder, Employee employee) {
        ...
        if (existingOrder == null) {
            throw new GearUpException(HttpStatus.NOT_FOUND, "Order with the specified ID does not exist.");
        }
        ...
        if (price < 0) {
            throw new GearUpException(HttpStatus.BAD_REQUEST, "Price must be non-negative.");
        }
