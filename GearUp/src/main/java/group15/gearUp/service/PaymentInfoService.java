...
import group15.gearUp.model.PaymentInfo;
import group15.gearUp.repository.PaymentInfoRepository;
...
public class PaymentInfoService {
     ...
     * @throws GearUpException if creation request is invalid or customer is not found
     */
    @Transactional
    public PaymentInfo createPaymentInfo(String aCardNumber, Date aExpiryDate, int aCvv, String aBillingAddress, Customer aCustomer){
        if (aCardNumber == null || aCardNumber.length() != 16 || !aCardNumber.matches("\\d+") || aExpiryDate == null || aExpiryDate.before(new Date(System.currentTimeMillis())) || String.valueOf(aCvv).length() != 3 || aBillingAddress == null || aBillingAddress.isEmpty()) {
            throw new GearUpException(HttpStatus.BAD_REQUEST, "Payment Card information is invalid.");
        }
        if (aCustomer == null) {
            throw new GearUpException(HttpStatus.BAD_REQUEST, "Customer must be valid and registered.");
        }
        Customer customer = customerRepo.findByUserID(aCustomer.getUserID());
    
        PaymentInfo paymentInfo = new PaymentInfo(aCardNumber, aExpiryDate, aCvv, aBillingAddress, customer);
    ...
    public PaymentInfo updatePaymentInfo(int paymentInfoId, PaymentInfo updatedPaymentInfo, Customer customer) {
        ...
        if (existingPaymentInfo == null) {
            throw new GearUpException(HttpStatus.NOT_FOUND, "Payment info with the specified ID does not exist.");
        }
        ...
        if (cardNumber == null || cardNumber.length() != 16 || !cardNumber.matches("\\d+") || expiryDate == null || expiryDate.before(new Date(System.currentTimeMillis())) || cvv <= 0 || String.valueOf(cvv).length() != 3 || billingAddress == null || billingAddress.isEmpty()) {
            throw new GearUpException(HttpStatus.BAD_REQUEST, "Payment Card information is invalid.");
        }
     ...
     * @throws GearUpException if payment info with given ID is not found
     */
    @Transactional
    public PaymentInfo getPaymentInfoById(int id){
        PaymentInfo paymentInfo = paymentInfoRepo.findByPaymentInfoID(id);
        if (paymentInfo == null){
            throw new GearUpException(HttpStatus.NOT_FOUND, "Payment Information not found.");
        }
        return paymentInfo;
    }
     ...
     * @throws GearUpException if the payment info with the given card number is not found
     */
    @Transactional
    public PaymentInfo getPaymentInfoByCardNumber(String cardNumber) {
        if (cardNumber == null || cardNumber.length() != 16) {
            throw new GearUpException(HttpStatus.BAD_REQUEST, "Invalid card number");
        }
     ...
     * @throws GearUpException if no payment info found
     */
    @Transactional
    public List<PaymentInfo> GetAllPaymentInfo(){
        List<PaymentInfo> paymentInfos = paymentInfoRepo.findAll();
        if(paymentInfos.size() == 0){
            throw new GearUpException(HttpStatus.NO_CONTENT, "No payment information found in the system.");
        }
        return paymentInfos;
    }
     ...
     * @throws GearUpException if payment info not found or unauthorized customer
     */
    @Transactional
    public void deletePaymentInfo(String cardNumber, Customer customer){
        PaymentInfo paymentInfo = paymentInfoRepo.findByCardNumber(cardNumber);
        if (paymentInfo == null) {
            throw new GearUpException(HttpStatus.NOT_FOUND, "Payment info with the specified card number does not exist.");
        }
        if (paymentInfo.getCustomer() == null || paymentInfo.getCustomer().getUserID() != customer.getUserID()) {
            throw new GearUpException(HttpStatus.FORBIDDEN, "Unauthorized access. Only the owner can delete this payment info.");
        }
