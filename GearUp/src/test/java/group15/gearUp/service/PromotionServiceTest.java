...
public class PromotionServiceTest {
    ...
    void testCreatePromotion_Success() {
        ...
        Promotion mockPromotion = new Promotion("PROMO2024", 20.0, validUntil, equipment);

        when(equipmentRepository.findEquipmentByEquipmentID(anyInt())).thenReturn(equipment);
        when(promotionRepository.save(any(Promotion.class))).thenReturn(mockPromotion);

        // Act
        Promotion createdPromotion = promotionService.createPromotion("PROMO2024", 20.0, validUntil, equipment);
        ...
        assertEquals(equipment, createdPromotion.getEquipment());
    ...
    void testDeletePromotion_Success() {
        ...
        Promotion mockPromotion = new Promotion(promotionCode, 20.0, Date.valueOf("2024-12-31"), equipment);
    ...
    void testDeletePromotion_Unauthorized() {
        // Arrange
        String promotionCode = "PROMO2024";
        Equipment authorizedEquipment = new Equipment();
        authorizedEquipment.setEquipmentID(1);
        
        Equipment unauthorizedEquipment = new Equipment();
        unauthorizedEquipment.setEquipmentID(2);

        Promotion promotion = new Promotion();
        promotion.setPromotionCode(promotionCode);
        promotion.setEquipment(authorizedEquipment);

        when(promotionRepository.findByPromotionCode(promotionCode)).thenReturn(promotion);

        // Act
        GearUpException thrown = assertThrows(GearUpException.class, () -> {
            promotionService.deletePromotion(promotionCode, unauthorizedEquipment);
        });

        // Assert
        assertEquals(HttpStatus.FORBIDDEN, thrown.getStatus());
        assertEquals("Unauthorized access. Only the associated equipment can delete this promotion.", thrown.getMessage());
    ...
    void testCreatePromotion_WithInvalidDiscount_ThrowsException() {
        ...
        // Act & Assert
        GearUpException thrown = assertThrows(GearUpException.class, () -> {
            promotionService.createPromotion("PROMO2024", -10.0, validUntil, equipment);
        });
...
void testCreatePromotion_WithDiscountOver100_ThrowsException() {
    ...
    // Act & Assert
    GearUpException thrown = assertThrows(GearUpException.class, () -> {
        promotionService.createPromotion("PROMO2024", 120.0, validUntil, equipment);
    });
    assertEquals("Discount percentage must be between 0 and 100.", thrown.getMessage());
}
...
void testCreatePromotion_WithExpiredDate_ThrowsException() {
    ...
    // Act & Assert
    GearUpException thrown = assertThrows(GearUpException.class, () -> {
        promotionService.createPromotion("PROMO2024", 20.0, expiredDate, equipment);
    });
...
void testUpdatePromotion_Success() {
    ...
    Promotion existingPromotion = new Promotion("OLDPROMO", 10.0, Date.valueOf("2024-12-31"), equipment);
    existingPromotion.setPromotionID(1);

    Promotion updatedPromotion = new Promotion("NEWPROMO", 20.0, Date.valueOf("2025-12-31"), equipment);

    when(promotionRepository.findById(1));
    when(promotionRepository.save(existingPromotion)).thenReturn(existingPromotion);

    // Act
    Promotion result = promotionService.updatePromotion(1, updatedPromotion, equipment);
...
void testUpdatePromotion_BadRequest() {
    ...
    Promotion existingPromotion = new Promotion("PROMO2024", 15.0, Date.valueOf("2024-12-31"), equipment);
    existingPromotion.setPromotionID(1);

    // Invalid updated promotion with empty promotion code and out-of-range discount percentage
    Promotion invalidUpdatedPromotion = new Promotion("", 150.0, Date.valueOf("2025-12-31"), equipment);

    when(promotionRepository.findById(1));

    // Act & Assert
    GearUpException exception = assertThrows(GearUpException.class, () -> {
        promotionService.updatePromotion(1, invalidUpdatedPromotion, equipment);
    });
...
void testUpdatePromotion_NotFound() {
    ...
    Promotion updatedPromotion = new Promotion("NEWPROMO2025", 20.0, Date.valueOf("2025-12-31"), equipment);

    when(promotionRepository.findById(1)).thenReturn(null); // Promotion with ID 1 does not exist

    // Act & Assert
    GearUpException exception = assertThrows(GearUpException.class, () -> {
        promotionService.updatePromotion(1, updatedPromotion, equipment);
    });
...
void testGetByPromotionCode_Success() {
    // Arrange
    Promotion promotion = new Promotion("PROMO2024", 20.0, Date.valueOf("2024-12-31"), new Equipment());
...
void testGetByValidUntil_Success() {
    // Arrange
    Date validUntil = Date.valueOf("2024-12-31");
    Promotion promotion = new Promotion("PROMO2024", 20.0, validUntil, new Equipment());
...
void testGetAllPromotion_Success() {
    // Arrange
    Promotion promotion1 = new Promotion("PROMO2024", 20.0, Date.valueOf("2024-12-31"), new Equipment());
    Promotion promotion2 = new Promotion("PROMO2025", 15.0, Date.valueOf("2025-12-31"), new Equipment());
